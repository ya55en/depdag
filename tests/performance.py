...
def perf_test():
    size = 3000
    no_cycle_check_time = timeit(lambda: create_large_dag(size), number=1)
    cycle_check_time = timeit(lambda: create_large_dag(size, True), number=1)
    ...
    print("Second is", int(cycle_check_time/no_cycle_check_time), "times slower.")
